...
(() => {
	  ...
	  case "object":
		if (Array.isArray(r))
		  ...
		  if (Symbol.toHtmlTag in r && typeof r[Symbol.toHtmlTag] === "function") {
			let y = String(r[Symbol.toHtmlTag]());
  ...
  ${m(this.parentTags.filter((r) => r !== ""), (r) => `<${r}>`).join(`
  `)}
  ...
  class U {
	...
	constructor(r, g = {}) {
	  ...
	  let y = this.window.document, P = this;
  
	  class J {
		createElement(b, S, H) {
		  let T = y.createElement(b);
		  for (let [X, s] of S)
		...
		createFragment(b) {
		  let S = y.createDocumentFragment();
		  for (let H of b)
			S.appendChild(H);
		  return S;
		}
  ...
  `)
	...
	static concatHtmlLines(r, g) {
	  let y = "", P = g?.indent ?? 2, J = " ".repeat(P), b = "", S = 0;
	  for (let H = 0;H < r.length; H++) {
		let T = r[H];
		if (S > 0) {
		  let s = r[H + 1]?.type === "tag-close" && r[H + 1]?.tag === "pre" ? "" : `
  `;
		  switch (T.type) {
			case "tag-open":
			  if (T.tag === "pre")
  ...
  `;
			...
			case "text": {
			  let X = V.trimContent(T.content), s = r[H + 1];
			  if (y.at(-1) === `
  `)
  ...
  class a extends V {
	...
	createElement(r, g, y) {
	  let P = F(g), J = this.flattenChildrenCompact(y);
  ...
  class Z extends V {
	...
	createElement(r, g, y) {
	  let P = F(g), J = this.flattenChildren(y);
  ...
  var _ = (r, g, y = {}) => {
	let { pretty: P = !1 } = y;
	if (P) {
	  let b = new E(new Z(y), { ...y, allowAsync: !1 }, g).render(r);
  ...
  class q {
	...
	update(r, g) {
	  ...
	  if (typeof y === "object" && y !== null && typeof g === "object" && g !== null)
		if (Array.isArray(y)) {
		  let P = Array.from(y), J = Object.entries(g);
		  for (let b = 0;b < J.length; b++) {
			let [S, H] = J[b], T = Number(S);
			if (!isNaN(T))
			  P[T] = H;
		  }
  ...
  class j {
	...
	matchMap(r, g) {
	  ...
	  for (let J = 0;J < r.length; J++) {
		let b = r[J], S = g(b, (H, T) => this.match(H, T));
		if (K(S)) {
		  if (this.options.allowAsync === !1)
			O();
		  P.push(S.then((H) => {
  ...
  class E {
	...
	constructor(r, g = { allowAsync: !1 }, y = f.create(g)) {
	  ...
	  this.matcher.functionTag((J, b) => {
		let S = J.funcComponent(J.props, b.componentApi);
		return P.renderChild(S, b);
	  }).classTag((J, b) => {
		let H = new J.classComponent(J.props).render(J.props, b.componentApi);
		return P.renderChild(H, b);
	  }).stringTag((J, b) => {
		let { attributes: S, children: H } = this.resolveProps(J.props), T = this.matcher.matchMap(H, (X, s) => s(X, {
		  componentApi: f.clone(b.componentApi)
		}));
		return this.generator.createElement(J.tag, S, T);
	  }).fragment((J, b) => {
		let S = Array.isArray(J) ? J.flat(1) : [J], H = this.matcher.matchMap(S, (T, X) => X(T, {
		  componentApi: f.clone(b.componentApi)
		}));
  ...
  class I extends V {
	createElement(r, g, y) {
	  return Promise.resolve(y).then((P) => Promise.all(P)).then((P) => {
		let J = F(g), b = this.flattenChildrenCompact(P);
  ...
  class A extends V {
	createElement(r, g, y) {
	  return Promise.resolve(y).then((P) => Promise.all(P)).then((P) => {
		let J = F(g), b = this.flattenChildren(P);
  ...
  var k = (r, g, y = {}) => {
	let { pretty: P = !1 } = y;
	if (P) {
	  let b = new E(new A(y), { ...y, allowAsync: !0 }, g).render(r);
	  return Promise.resolve(b).then((S) => A.concatHtmlLines(S, y));
	} else {
	  let J = new E(new I(y), { ...y, allowAsync: !0 }, g);
